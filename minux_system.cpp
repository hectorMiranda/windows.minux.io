...
void StartPerformanceCounters() {
    ...
    PdhAddEnglishCounter(g_hQuery, "\\Processor(_Total)\\% Processor Time", 0, &g_hCpuCounter);
...
SystemInfo GetSystemInformation() {
    ...
    // Get computer name
    DWORD nameSize = sizeof(info.computerName);
    GetComputerNameA(info.computerName, &nameSize);
    
    // Get OS version
    OSVERSIONINFOEXA osvi = {0};
    osvi.dwOSVersionInfoSize = sizeof(osvi);
    if (GetVersionExA((OSVERSIONINFOA*)&osvi)) {
        sprintf_s(info.osVersion, "Windows %d.%d Build %d", 
...
std::vector<ProcessInfo> GetProcessList() {
    ...
    if (Process32First(hSnapshot, &pe32)) {
        do {
            ...
            strcpy_s(info.processName, pe32.szExeFile);
...
std::vector<NetworkInfo> GetNetworkInterfaces() {
    ...
    if (GetAdaptersInfo(NULL, &dwSize) == ERROR_BUFFER_OVERFLOW) {
        ...
        if (pAdapterInfo && GetAdaptersInfo(pAdapterInfo, &dwSize) == NO_ERROR) {
            ...
            while (pAdapter) {
                NetworkInfo info = {0};
                strcpy_s(info.adapterName, pAdapter->AdapterName);
                strcpy_s(info.ipAddress, pAdapter->IpAddressList.IpAddress.String);
                
                // Format MAC address
                sprintf_s(info.macAddress, "%02X-%02X-%02X-%02X-%02X-%02X",
                         pAdapter->Address[0], pAdapter->Address[1], pAdapter->Address[2],
                         pAdapter->Address[3], pAdapter->Address[4], pAdapter->Address[5]);
                
                info.isConnected = (strcmp(info.ipAddress, "0.0.0.0") != 0);
